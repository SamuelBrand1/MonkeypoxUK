...
@save(
    "posteriors/posterior_mdl_select_param_draws_" * string(wks[end]) * ".jld2",
    param_draws
)
detected_cases = [particle.other.detected_cases for particle in smc_mdl_select.particles]
@save(
    "posteriors/posterior_mdl_select_detected_cases_" * string(wks[end]) * ".jld2",
    detected_cases
)
...
@save(
    "posteriors/posterior_mdl_select_incidences_" * string(wks[end]) * ".jld2",
    incidences
)
susceptibilities = [particle.other.susceptibility for particle in smc_mdl_select.particles]
@save(
    "posteriors/posterior_mdl_select_susceptibilities_" * string(wks[end]) * ".jld2",
    susceptibilities
)
end_states = [particle.other.end_state for particle in smc_mdl_select.particles]
@save(
    "posteriors/posterior_mdl_select_end_states_" * string(wks[end]) * ".jld2",
    end_states
)
begin_vac_states =
    [particle.other.state_pre_vaccine for particle in smc_mdl_select.particles]
@save(
    "posteriors/posterior_mdl_select_begin_vac_states_" * string(wks[end]) * ".jld2",
    begin_vac_states
)
begin_sept_states = [particle.other.state_sept for particle in smc_mdl_select.particles]
@save(
    "posteriors/posterior_mdl_select_begin_sept_states_" * string(wks[end]) * ".jld2",
    begin_sept_states
)
vac_effectivenesses =
    [particle.other.vac_effectiveness for particle in smc_mdl_select.particles]
@save(
    "posteriors/posterior_mdl_select_vac_effectivenesses_" * string(wks[end]) * ".jld2",
    vac_effectivenesses
)
