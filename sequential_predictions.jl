...
wks =
    Date.(
        past_mpxv_data_inferred_latest.week[1:size(mpxv_wkly, 1)],
        DateFormat("dd/mm/yyyy"),
    )

inferred_prop_na_msm = past_mpxv_data_inferred_latest[:, colname] |> x -> x[.~ismissing.(x)]
inferred_prop_na_msm_lwr =
    past_mpxv_data_inferred_latest[:, "lower_"*colname] |> x -> x[.~ismissing.(x)]
inferred_prop_na_msm_upr =
    past_mpxv_data_inferred_latest[:, "upper_"*colname] |> x -> x[.~ismissing.(x)]



mpxv_wkly =
    Matrix(
        past_mpxv_data_inferred_latest[
            1:size(inferred_prop_na_msm, 1),
            ["gbmsm", "nongbmsm"],
        ],
    ) .+
