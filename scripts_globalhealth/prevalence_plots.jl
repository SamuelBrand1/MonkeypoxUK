...
for grp = 1:10
    prevalence[:, "Prev. % GBMSM sx activity grp $(grp) (post. mean; 12wk reversion)"] =
        100 .* prev_cred_int_12wkrev.mean_pred[3:end, grp] ./ N_msm_grp[grp]
    prevalence[:, "Prev. % GBMSM sx activity grp $(grp) (post. 10%; 12wk reversion)"] =
        100 .* (
            prev_cred_int_12wkrev.mean_pred[3:end, grp] .-
            prev_cred_int_12wkrev.lb_pred_10[3:end, grp]
        ) ./ N_msm_grp[grp]
    prevalence[:, "Prev. % GBMSM sx activity grp $(grp) (post. 90%; 12wk reversion)"] =
        100 .* (
            prev_cred_int_12wkrev.mean_pred[3:end, grp] .+
            prev_cred_int_12wkrev.ub_pred_10[3:end, grp]
        ) ./ N_msm_grp[grp]
end
...
for grp = 1:10
    prevalence[:, "Prev. % GBMSM sx activity grp $(grp) (post. mean; 4wk reversion)"] =
        100 .* prev_cred_int_4wkrev.mean_pred[3:end, grp] ./ N_msm_grp[grp]
    prevalence[:, "Prev. % GBMSM sx activity grp $(grp) (post. 10%; 4wk reversion)"] =
        100 .* (
            prev_cred_int_4wkrev.mean_pred[3:end, grp] .-
            prev_cred_int_4wkrev.lb_pred_10[3:end, grp]
        ) ./ N_msm_grp[grp]
    prevalence[:, "Prev. % GBMSM sx activity grp $(grp) (post. 90%; 4wk reversion)"] =
        100 .* (
            prev_cred_int_4wkrev.mean_pred[3:end, grp] .+
            prev_cred_int_4wkrev.ub_pred_10[3:end, grp]
        ) ./ N_msm_grp[grp]
end
...
prevalence[:, "Prev. % GBMSM overall (post. mean; 4wk reversion)"] =
    100 .* prev_cred_int_overall_4wkrev.mean_pred[3:end] ./ N_msm
prevalence[:, "Prev. % GBMSM overall (post. 10%; 4wk reversion)"] =
    100 .* (
        prev_cred_int_overall_4wkrev.mean_pred[3:end] .-
        prev_cred_int_overall_4wkrev.lb_pred_10[3:end]
    ) ./ N_msm
prevalence[:, "Prev. % GBMSM overall (post. 90%; 4wk reversion)"] =
    100 .* (
        prev_cred_int_overall_4wkrev.mean_pred[3:end] .+
        prev_cred_int_overall_4wkrev.ub_pred_10[3:end]
    ) ./ N_msm

prevalence[:, "Prev. % GBMSM overall (post. mean; 12wk reversion)"] =
    100 .* prev_cred_int_overall_12wkrev.mean_pred[3:end] ./ N_msm
prevalence[:, "Prev. % GBMSM overall (post. 10%; 12wk reversion)"] =
    100 .* (
        prev_cred_int_overall_12wkrev.mean_pred[3:end] .-
        prev_cred_int_overall_12wkrev.lb_pred_10[3:end]
    ) ./ N_msm
prevalence[:, "Prev. % GBMSM overall (post. 90%; 12wk reversion)"] =
    100 .* (
        prev_cred_int_overall_12wkrev.mean_pred[3:end] .+
        prev_cred_int_overall_12wkrev.ub_pred_10[3:end]
    ) ./ N_msm
...
prevalence[:, "Prev. % non-GBMSM overall (post. mean; 4wk reversion)"] =
    100 .* prev_cred_int_4wkrev.mean_pred[3:end, 11] ./ (N_uk - N_msm)
prevalence[:, "Prev. % non-GBMSM overall (post. 10%; 4wk reversion)"] =
    100 .* (
        prev_cred_int_4wkrev.mean_pred[3:end, 11] .-
        prev_cred_int_4wkrev.lb_pred_10[3:end, 11]
    ) ./ (N_uk - N_msm)
prevalence[:, "Prev. % non-GBMSM overall (post. 90%; 4wk reversion)"] =
    100 .* (
        prev_cred_int_4wkrev.mean_pred[3:end, 11] .+
        prev_cred_int_4wkrev.ub_pred_10[3:end, 11]
    ) ./ (N_uk - N_msm)

prevalence[:, "Prev. % non-GBMSM overall (post. mean; 12wk reversion)"] =
    100 .* prev_cred_int_12wkrev.mean_pred[3:end, 11] ./ (N_uk - N_msm)
prevalence[:, "Prev. % non-GBMSM overall (post. 10%; 12wk reversion)"] =
    100 .* (
        prev_cred_int_12wkrev.mean_pred[3:end, 11] .-
        prev_cred_int_12wkrev.lb_pred_10[3:end, 11]
    ) ./ (N_uk - N_msm)
prevalence[:, "Prev. % non-GBMSM overall (post. 90%; 12wk reversion)"] =
    100 .* (
        prev_cred_int_12wkrev.mean_pred[3:end, 11] .+
        prev_cred_int_12wkrev.ub_pred_10[3:end, 11]
    ) ./ (N_uk - N_msm)
