...
UK_mpxv_data = mpxv_data[
    [c ∈ ["England", "Wales", "Scotland", "Northern Ireland"] for c in mpxv_data.Country],
    :,
]
...
reported_msm_prop = [
    ...
    Date(2022, 8, 1) 0.953
] #https://www.gov.uk/government/publications/monkeypox-outbreak-technical-briefings/investigation-into-monkeypox-outbreak-in-england-technical-briefing-5



first_day_wk_reported =
    Dates.firstdayofweek.(UK_mpxv_data.Date_confirmation[idxs_confirmed])
wks = sort(unique(first_day_wk_reported))
mpxv_wkly =
    [sum(first_day_wk_reported .== wk) for wk in wks] .* [fill(reported_msm_prop[end, 2], length(wks)) fill(1 - 0.965, length(wks))]
...
days =
    minimum(unique(UK_mpxv_data.Date_confirmation)):Day(1):maximum(
        unique(UK_mpxv_data.Date_confirmation),
    )
...
shift = 5
grouped_mpxv_fri = [
    sum(_mpxv_dly[Dates.firstdayofweek.(days)+Day(shift).==wk]) for
    wk in (wks .+ Day(shift))
]
scatter(
    wks,
