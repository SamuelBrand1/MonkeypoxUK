...
@save(
    "posteriors/posterior_param_draws_" * string(wks[end]) * description_str * ".jld2",
    param_draws
)
detected_cases = [particle.other.detected_cases for particle in smc_cng_pnt.particles]
@save(
    "posteriors/posterior_detected_cases_" * string(wks[end]) * description_str * ".jld2",
    detected_cases
)
...
vac_effectivenesses =
    [particle.other.vac_effectiveness for particle in smc_cng_pnt.particles]
@save(
    "posteriors/posterior_vac_effectivenesses_" *
    string(wks[end]) *
    description_str *
    ".jld2",
    vac_effectivenesses
